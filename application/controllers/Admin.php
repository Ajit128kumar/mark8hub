...
//new function for map
    public  function getCityName()
    {
        $res = array();
        $city_id = $_POST['city_id'];
        $city = $this->crud_model->get_cities($city_id)->row_object();
        $city= $city->name;
        $res['city'] = $city;
...
//listing excel upload

 public function listingExcelUpload()
    {
        //error_reporting(1);
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        $country_id = $_POST['country_id_excel'];
        $city_id = $_POST['city_id_excel'];
        if(empty($country_id) || empty($city_id))
        {
            $this->session->set_flashdata('error_message', 'Please Select Country And City');
            redirect(site_url('admin/listing_form/add'), 'refresh');
        ...
        {
            $this->session->set_flashdata('error_message', 'Please upload a proper format of excel in xlsx or xls or csv');
            redirect(site_url('admin/listing_form/add'), 'refresh');
        }



        $name = time() . $_FILES['document']['name'];
        $folder_path = $_SERVER['DOCUMENT_ROOT'] . '/uploads/excel';
      //  echo $folder_path;exit;
        move_uploaded_file($_FILES['document']['tmp_name'], $folder_path . "/" . $name);
        $inputFileName = $_SERVER['DOCUMENT_ROOT'] . "/uploads/excel/" . $name;
        
        /** @var TYPE_NAME $objPHPExcel */
...
//
        ...
        for ($i = 2; $i <= $arrayCount; $i++) {
            if (!empty(trim($allDataInSheet[$i]["A"])) && !empty(trim($allDataInSheet[$i]["C"])) && !empty(trim($allDataInSheet[$i]["I"]))) {
                $categories= array();
                if(!empty(trim($allDataInSheet[$i]["C"])))
                {
                    array_push($categories,trim($allDataInSheet[$i]["C"]));
                }
                $_POST['name'] = trim($allDataInSheet[$i]["A"]);
                $_POST['country_id'] =$country_id;
                $_POST['city_id'] = $city_id;
                $_POST['description'] = trim($allDataInSheet[$i]["B"]);
                $_POST['categories'] = $categories;
                $_POST['is_featured'] = trim($allDataInSheet[$i]["D"]);
                $_POST['address'] = trim($allDataInSheet[$i]["E"]);
                $_POST['email'] = trim($allDataInSheet[$i]["F"]);
                $_POST['phone'] = trim($allDataInSheet[$i]["G"]);
                $_POST['website'] = trim($allDataInSheet[$i]["H"]);
                $_POST['listing_type'] = strtolower(trim($allDataInSheet[$i]["I"]));
                $_POST['type'] = trim($allDataInSheet[$i]["J"]);
                $_POST['bgroup'] = trim($allDataInSheet[$i]["K"]);
                $_POST['latitude'] = trim($allDataInSheet[$i]["L"]);
                $_POST['longitude'] = trim($allDataInSheet[$i]["M"]);
                $this->crud_model->add_listing_excel();
            }
        }
...
//listing excel upload ends

   //new
 
 public function listingExcelUploadBranch()
    {
        //error_reporting(1);
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        ...
        if (!empty($parent_id)) {
            $parent_listing = $this->crud_model->get_listings($parent_id)->row_array();
            $categories = json_decode($parent_listing['categories']);
            $type       = $parent_listing['type'];
            $bgroup       = $parent_listing['bgroup'];
            $listing_type       = $parent_listing['listing_type'];
...
//
        ...
        for ($i = 2; $i <= $arrayCount; $i++) {
            if (!empty(trim($allDataInSheet[$i]["A"]))) {
                ...
                $_POST['phone'] = trim($allDataInSheet[$i]["G"]);
                $_POST['website'] = trim($allDataInSheet[$i]["H"]);
                $_POST['listing_type'] = $listing_type;
                $_POST['type'] = $type;
                $_POST['bgroup'] = $bgroup;
    ...
    function booking_request_service($param1 ='', $param2 = ''){
        ...
        if($param1 == 'approved'){
            ...
            $this->email_model->request_approved_mail($param2);
            $this->session->set_flashdata('flash_message', get_phrase('request_approved_successfully'));
            redirect(site_url('admin/booking_request_service'), 'refresh');
        }
        if($param1 == 'pending'){
            $data['status'] = 0;
            $this->db->where('id', $param2);
