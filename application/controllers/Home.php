...
class Home extends CI_Controller {
  ...
  function featured_product()
    {
       // echo 'here';exit;
        $page_data['page_name']     =   'featured_product';
        $page_data['title']         =   'Featured product';
        $this->load->view('frontend/index', $page_data);
    }
 function featured_service()
    {
       // echo 'here';exit;
        $page_data['page_name']     =   'featured_service';
        $page_data['title']         =   'Featured service';
        $this->load->view('frontend/index', $page_data);
    }
    ...
    public function login() {
        ...
        $button = '<a href="'.$client->createAuthUrl().'">
                      <img height="40px" style="object-fit:contain" width="40px" class="mb-md-2" src="'.base_url().'uploads/google-imagesignin.png"/>
                     
                </a>';
                
            $page_data['button'] = $button;

      //google login ends
        //facebook login
        $data['user'] = array();
        if ($this->facebook->is_authenticated() && $this->session->set_userdata('is_logged_in')==1) {
            
        } else {
            $button_fb = '<div class="px-1" style=" border-left: thin solid lightgray;"><a href="'.$this->facebook->login_url().'"><img height="40px" style="object-fit:contain" width="40px" src="'.base_url().'uploads/facebook-imagesignin.png"/></a></div>';
            $page_data['button_fb'] = $button_fb;
        }
...
// echo $listing_id;die();
        $branches = $this->db->get_where('business_branch', array('listing_id' => $listing_id))->result_array();
        // if(!empty())
        $branch_geo_json = $this->make_branch_geo_json_for_map($branches);
        $page_data['branches_geo_json'] = $branch_geo_json;
        $page_data['branches'] = $branches;
    ...
    function make_branch_geo_json_for_map($branches = array(), $param2 = "", $listing_id = ""){
        // get all file names in all-listings-geojson folder
        ...
        //     chmod("assets/frontend/single-listing-geojson/listing-id-".$listing_id.".json", 0777);
        // else:
            $sum_of_listing_id = count($branches);
            foreach ($branches as $branch) {
                $sum_of_listing_id += $branch['id'];
            }
            if(!file_exists('assets/frontend/all-branches-geojson/branch-id-'.$sum_of_listing_id.'.json')){
                fopen('assets/frontend/all-branches-geojson/branch-id-'.$sum_of_listing_id.'.json', 'w');
            }

            chmod("assets/frontend/all-branches-geojson/branch-id-".$sum_of_listing_id.".json", 0777);

        // endif;
        $listing_details_array = array();
        // echo "<pre>";
        // print_r($branches);die();
        foreach ($branches as $key => $listing) {
            // echo "<pre>";
            // print_r($listing);die();
            $listing_details = array();
            // $listing_tags = explode(',', $listing['tags']);
        ...
        if($param2 == 'listing_single_page'):
            ...
            file_put_contents("assets/frontend/all-branches-geojson/branch-id-".$sum_of_listing_id.".json", stripslashes($jsonData));
        endif;
        return json_encode($listing_geo_array);
    }
    ...
    function contact_service() {
      //  echo 'here baby';exit;
        ...
        $listing_details = $this->crud_model->get_listing_details($listing_id)->row_array();
            ...
            $data['to'] = $listing_details['email'];
            if ($data['date'] != "" && $data['time'] != "") {
                $this->frontend_model->service();
                $this->email_model->service_mail($data);
            }else {
                $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
    ...
    function ask_quotation() {
        ...
        $data['product'] = sanitizer($this->input->post('product'));
        $data['note'] = sanitizer($this->input->post('note'));
        $data['to'] = $listing_details['email'];
        ...
        if ($data['service'] != "" || $data['product'] != "") {
            ...
            $this->frontend_model->service();
            $this->email_model->quotation_mail($data);
        }else {
            $this->session->set_flashdata('error_message', get_phrase('fill_all_fields_first'));
    ...
    function search($page_number = 1) {
        ...
        $all_listings = $this->frontend_model->search_listing_all_rows_new($search_string, $selected_category_id,$selected_city_id);
        $listings = $this->frontend_model->search_listing_new($search_string, $selected_category_id, $page_number,$selected_city_id);
        $geo_json = $this->make_geo_json_for_map($listings);
    ...
    function searchHome($page_number = 1) {
        $search_string = $_GET['search_string'];
    ...
    function service_time($param1 = '', $param2 = ''){
        $this->crud_model->service_checking_time($param1, $param2);
    }
