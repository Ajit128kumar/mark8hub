...
//new functions
  ...
  function search_listing_all_rows_new($search_string = '', $selected_category_id = '',$selected_city='') {
       $search_sql='';
        if ($search_string != "") {
            $search_sql.= ' AND ( name like "%'.$search_string.'%"
            ) ';
        }

        if ($selected_category_id != "") {
           $search_sql.=' AND  categories like \'%'.'"'.$selected_category_id.'"'.'%\'';
        }
        if ($selected_city != "") {
           $search_sql.=' AND city_id ='.$selected_city;
        }
      $sql = 'SELECT * FROM listing WHERE status="active" '.$search_sql.' ORDER BY is_featured DESC';
...
//echo $sql;exit;
      ...
      function search_listing_new($search_string = '', $selected_category_id = '', $page_number = 1,$selected_city='') {
        if($page_number <= 1):
            $starting_value = 0;
        else:
        ...
        $sql = 'SELECT * FROM listing WHERE status="active" '.$search_sql.' ORDER BY is_featured DESC LIMIT 12 OFFSET '.$starting_value;
        $query = $this->db->query($sql);
//echo $sql;exit;
        if ($query->num_rows() >= 1) {
...
//new function ends
    ...
    function claim_this_listing() {
      ...
      $data['status'] = 0;
       if ($_FILES['pan']['name'] == "") {
            ...
            move_uploaded_file($_FILES['pan']['tmp_name'], 'uploads/claimed_pan/' . $data['pan']);
        }
    ...
    function get_top_ten_listings() {
        ...
        foreach ($listing_id_with_rating as $key => $value) {
            if (count($listing_ids) <= 100) {
                array_push($listing_ids, $key);
            }
        }
        if (count($listing_ids) > 0) {
            $this->db->select('listing.*, country.name as country_name,city.name as city_name');
            $this->db->where_in('listing.id', $listing_ids);
            $this->db->where('status', 'active');
            $this->db->join('country', 'country.id = listing.country_id','left');
            $this->db->join('city', 'city.id = listing.city_id','left');
