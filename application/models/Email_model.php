...
class Email_model extends CI_Model {
    ...
    public function service_mail($data = "") {
        $date = date('D, d-M-Y', strtotime($data['date']));
        $subject 		= "Service Booking Request on $date";
        $user_details = $this->user_model->get_all_users($this->session->userdata('user_id'))->row_array();
        $email_msg	=	"<b>Hello,</b>";
        $email_msg	.=	"<p>Time : ".$this->input->post('time')."</p>";
        $email_msg  .=  "<p>Phone: ".$user_details['phone']."</p>";
        $email_msg	.=	"<p>Service : ".$this->db->get_where('service', array('id' => $this->input->post('service')))->row('name')."</p>";
        $email_msg	.=	"<p>I need this service. Please accept my request and let me know from your side.</p>";

        $this->send_smtp_mail($email_msg, $subject, $data['to'], $user_details['email']);
        //$this->sent_smtp_mail_with_php_mailer_library($email_msg, $subject, $data['to'], $user_details['email']);
    }

    public function quotation_mail($data = "") {
        	
        $subject        = "Quotation Form Request";
        $user_details = $this->user_model->get_all_users($this->session->userdata('user_id'))->row_array();
        $email_msg  =   "<b>Hello,</b>";
        
        $email_msg  .=  "<p>User Email: ".$user_details['email']."</p>";
        $email_msg  .=  "<p>User Phone: ".$user_details['phone']."</p>";
        if(!empty($data['service'])){
            $email_msg  .=  "<p>Service : ".$data['service']."</p>";
        }
        if(!empty($data['product'])){
           $email_msg  .=  "<p>Product : ".$data['product']."</p>"; 
        }
        $email_msg  .=  "<p>Quantity : ".$data['quantity']."</p>";
        $email_msg  .=  "<p>Note : ".$data['note']."</p>";
