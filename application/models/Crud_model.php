...
//new
    ...
    function insert_branch($data){
        $this->db->insert("add_branch", $data);
    }
    function get_by_query($sql)
    {
    ...
    function get_listing_count_from_category($category_id = 0)
    {
        $sql = ' SELECT count(*) as count FROM listing WHERE categories like \'%' . '"' . $category_id . '"' . '%\'';
        //  echo $sql;exit;
        $query = $this->db->query($sql);
...
//new view branch
    function get_branches($parent_id = 0)
    {
        $this->db->where('parent_id', $parent_id);
        return $this->db->get('listing')->result_array();
    }

    function get_new_branches($parent_id = 0)
    {
        $this->db->where('listing_id', $parent_id);
        return $this->db->get('business_branch')->result_array();
    }

    function get_new_achievements($parent_id = 0)
    {
        $this->db->where('listing_id', $parent_id);
        return $this->db->get('business_achievements')->row_array();
    }
...
//new ends
    ...
    function add_listing()
    {
        ...
        $data['type'] = sanitizer($this->input->post('type'));
        $data['bgroup'] = sanitizer($this->input->post('bgroup'));
        if (!empty($this->input->post('parent_id'))) {
            $data['parent_id'] = sanitizer($this->input->post('parent_id'));
        }

        $data['google_analytics_id'] = sanitizer($this->input->post('google_analytics_id'));

        if(!empty(sanitizer($this->input->post('amenities')))){
        ...
        if(!empty(sanitizer($this->input->post('categories')))){
        ...
        //brand new
        $brand_array = array();
        $brand_names = $this->input->post('brand_name');
//        echo '<pre>';
//        print_r($brand_names);
//        echo '</pre>';exit;
        $brand_logos = $_FILES['brand_logo'];
        $count_brand = count($brand_names) - 1;
        //  echo $count_brand;exit;
        for ($k = 0; $k < $count_brand; $k++) {
            $brand_name = $brand_names[$k];
            //echo $brand_name;exit;
            //  print_r($brand_logos);exit;
            $brand_image = $brand_logos['tmp_name'][$k];
            if ($brand_image != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($brand_image, 'uploads/brand_images/' . $random_identifier);
                $brand_array[$brand_name] = $random_identifier;
            }
        }
        //   echo 'end';exit;
        $data['brands'] = json_encode($brand_array);
        //brand new ends
        ...
        if ($_FILES['pan']['name'] == "") {
            $data['pan'] = 'thumbnail.png';
        } else {
            $data['pan'] = md5(rand(10000000, 20000000)) . '.jpg';
            move_uploaded_file($_FILES['pan']['tmp_name'], 'uploads/pan/' . $data['pan']);
        }
        ...
        if ($_FILES['logo']['name'] == "") {
            ...
            move_uploaded_file($_FILES['logo']['tmp_name'], 'uploads/logo/' . $data['logo']);
        }
        ...
        if ($total_listing > $submited_listing || $user_type == '1') {
            ...
            $this->add_listing_service_details($listing_id);
...
//save listing excel
    ...
    function add_listing_excel()
    {
        ...
        $days = array('sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday');
        foreach ($days as $day) {
            $time_config[$day] = sanitizer('closed') . '-' . sanitizer('closed');
        }
        ...
        if ($user_type == '1') {
            ...
            $this->session->set_flashdata('error_message', 'Only Admin Can Upload Bulk');
        }
    }

//save listing excel ends
//new fn to update listing infos
    function update_listing_info($listing_id = "")
    {
        ...
        if (!empty($this->input->post('new_listing_images'))) {
        ...
        //new edit brand
        if (!empty($this->input->post('new_brand_images'))) {
            $brand_array = array();
            $brand_names = $this->input->post('brand_name');
            $brand_logos = $_FILES['brand_logo'];
            $count_brand = count($brand_names) - 1;
            $old_brand_images = json_decode($listing_details['brands']);
            $new_brand_images = $this->input->post('new_brand_images');
            unset($new_brand_images[count($new_brand_images) - 1]);
            $final_brand_images = array();
            ...
            for ($k = 0; $k < $count_brand; $k++) {
                ...
                if (in_array($new_brand_images[$brand_name], $old_brand_images)) {
                    //   echo 'here';exit;
                    if ($brand_image != "") {
                        $random_identifier = $new_brand_images[$brand_name];
                        move_uploaded_file($brand_image, 'uploads/brand_images/' . $random_identifier);
                        $brand_array[$brand_name] = $random_identifier;
                    } else {
                        $random_identifier = $new_brand_images[$brand_name];
        ...
        // Function
        $this->update_listing_service_details($listing_id);
        $this->session->set_flashdata('flash_message', 'Listing updated successfully');
    }

    function update_achievements($listing_id){

       
        $result = $this->db->get_where('business_achievements', array('listing_id' => $listing_id))->row_array();
        // echo "<pre>";print_r($result);die();
         if (!empty($this->input->post('new_brand_images')) && count(array_filter($this->input->post('new_brand_images'))) != 0) {
           
            $brand_array = array();
            $brand_names = $this->input->post('brand_name');
            $brand_logos = $_FILES['brand_logo'];
            $count_brand = count($brand_names) - 1;
            // echo $count_brand;die();
            $old_brand_images = json_decode($result['achievement']);
            // $old_brand_images = $this->input->post('old_brand_images');
            $new_brand_images = $this->input->post('new_brand_images');
            unset($new_brand_images[count($new_brand_images) - 1]);
            $final_brand_images = array();
            ...
            for ($k = 0; $k < $count_brand; $k++) {
                ...
                if (in_array($new_brand_images[$brand_name], $old_brand_images)) {
                    ...
                    if ($brand_image != "") {
                        ...
                        // $brand_array[$brand_name] = 
                        // $random_identifier = $new_brand_images[$brand_name];
                        $brand_array[$brand_name] = $new_brand_images[$k];
            ...
            $old_brand_images = json_decode(json_encode($old_brand_images), true);

            $data['achievement'] = json_encode($brand_array);

            $this->db->where('listing_id', $listing_id);
            $this->db->update('business_achievements', $data);
            

            $this->session->set_flashdata('flash_message', 'Achievements updated successfully');
        }else{
            ...
            $this->db->insert('business_achievements',$data);

            $this->session->set_flashdata('flash_message', 'Achievements added successfully');
        }


    }

    //new fn to update listing infos ends

    function update_branch($listing_id = ""){

        $branch_ids_array      = $this->input->post('new_branch_id');
        // array_pop($branch_id);
        
        $branch_name_array      = $this->input->post('business_name');
        array_pop($branch_name_array);

        $branch_contact_array   = $this->input->post('contact_no');
        array_pop($branch_contact_array);

        $branch_email_array     = $this->input->post('email');
        array_pop($branch_email_array);

        $branch_address_array   = $this->input->post('address');
        array_pop($branch_address_array);

        $branch_lat_array       = $this->input->post('latitude');
        array_pop($branch_lat_array);

        $branch_lon_array       = $this->input->post('longitude');
        array_pop($branch_lon_array);

        foreach ($branch_ids_array as $key => $branch_id) {
            $branchData['listing_id']       = sanitizer($listing_id);
            $branchData['branchName']       = sanitizer($branch_name_array[$key]);
            $branchData['contactNumber']    = sanitizer($branch_contact_array[$key]);
            $branchData['email']            = sanitizer($branch_email_array[$key]);
            $branchData['address']          = sanitizer($branch_address_array[$key]);
            $branchData['lat']              = sanitizer($branch_lat_array[$key]);
            $branchData['lon']              = sanitizer($branch_lon_array[$key]);
            if ($branch_id > 0) {
                $this->db->where('id', $branch_id);
                $this->db->update('business_branch', $branchData);
            } else {
                $this->db->insert('business_branch', $branchData);
            }
            // $this->db->insert('business_branch', $branchData);
        }
        $this->session->set_flashdata('flash_message', get_phrase('branches Updated successfully.'));
    }

    function add_listing_service_details($listing_id = "")
    {
        $listing_photos = array();
        $service_name_array = sanitizer($this->input->post('new_service_name'));
        array_pop($service_name_array);

        $service_starting_time_array = $this->input->post('new_starting_time');
        array_pop($service_starting_time_array);

        $service_ending_time_array = $this->input->post('new_ending_time');
        array_pop($service_ending_time_array);

        $service_negotiable_array = sanitizer($this->input->post('new_negotiable'));
        array_pop($service_negotiable_array);

        $service_duration_array = $this->input->post('new_duration');
        array_pop($service_duration_array);

        $service_price_array = sanitizer($this->input->post('new_service_price'));
        array_pop($service_price_array);

        $service_description_array = sanitizer($this->input->post('new_description'));
        array_pop($service_description_array);

        foreach ($_FILES['new_service_image']['tmp_name'] as $service_image) {
            if ($service_image != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image, 'uploads/service_images/' . $random_identifier);
                array_push($listing_photos, $random_identifier);
            }
        }
//echo '<pre>';
//        print_r($listing_photos);
//echo '</pre>';exit;
        foreach ($service_name_array as $key => $service_name) {
            ...
            $beauty_service_data['description'] = sanitizer($service_description_array[$key]);
            ...
            $beauty_service_data['negotiable'] = sanitizer($service_negotiable_array[$key]);
            $this->db->insert('service', $beauty_service_data);
        }
...
// This function saves listing wise inner data
    ...
    function update_listing($listing_id = "")
    {
        ...
        if ($_FILES['pan']['name'] != "") {
            if ($listing_details['pan'] == "thumbnail.png" || $listing_details['pan'] == "") {
                $data['pan'] = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($_FILES['pan']['tmp_name'], 'uploads/pan/' . $data['pan']);
            } else {
                $data['pan'] = $listing_details['pan'];
                move_uploaded_file($_FILES['pan']['tmp_name'], 'uploads/pan/' . $data['pan']);
            }
        }

        if ($_FILES['logo']['name'] != "") {
            if ($listing_details['logo'] == "thumbnail.png" || $listing_details['logo'] == "") {
                $data['logo'] = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($_FILES['logo']['tmp_name'], 'uploads/logo/' . $data['logo']);
            } else {
                $data['logo'] = $listing_details['logo'];
                move_uploaded_file($_FILES['logo']['tmp_name'], 'uploads/logo/' . $data['logo']);
            }
        }
...
// This function updates listing wise inner data
    function update_listing_service_details($listing_id = "")
    {

    
        $listing_photos_1 = array();
        $listing_photos_2 = array();
        $listing_photos_3 = array();
        $listing_photos_4 = array();
        $listing_photos_5 = array();
        // Updating listing wise image and data saving.

        $service_name_array = sanitizer($this->input->post('new_service_name'));
        array_shift($service_name_array);

        $service_starting_time_array = $this->input->post('new_starting_time');
        array_shift($service_starting_time_array);

        $service_ending_time_array = $this->input->post('new_ending_time');
        array_shift($service_ending_time_array);

        $service_duration_array = $this->input->post('new_duration');
        array_shift($service_duration_array);

        $service_price_array = sanitizer($this->input->post('new_service_price'));
        array_shift($service_price_array);

        $service_description_array = sanitizer($this->input->post('new_description'));
        array_shift($service_description_array);

        $service_negotiable_array = sanitizer($this->input->post('new_negotiable'));
        array_shift($service_negotiable_array);

        $service_ids_array = sanitizer($this->input->post('new_service_id'));
...
//            echo '</pre>';exit;
        ...
        if(!empty($this->input->post('old_service_images_1'))){

            $old_beauty_service_images_1 = sanitizer($this->input->post('old_service_images_1'));
            array_shift($old_beauty_service_images_1);
        }

        if(!empty($this->input->post('old_service_images_2'))){
            
            $old_beauty_service_images_2 = sanitizer($this->input->post('old_service_images_2'));
            array_shift($old_beauty_service_images_2);
        }

        if(!empty($this->input->post('old_service_images_3'))){
            
            $old_beauty_service_images_3 = sanitizer($this->input->post('old_service_images_3'));
            array_shift($old_beauty_service_images_3);
        }

        if(!empty($this->input->post('old_service_images_4'))){
                
            $old_beauty_service_images_4 = sanitizer($this->input->post('old_service_images_4'));
            array_shift($old_beauty_service_images_4);
        }

        if(!empty($this->input->post('old_service_images_5'))){
            
            $old_beauty_service_images_5 = sanitizer($this->input->post('old_service_images_5'));
            array_shift($old_beauty_service_images_5);            
        }
        
            // echo "<pre>";
            // print_r($_FILES['new_service_image_1']['name']);die();
        // Image Uploading functions ends here

        foreach ($_FILES['new_service_image_1']['tmp_name'] as $key => $service_image_1) {
            if ($service_image_1 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_1, 'uploads/service_images/' . $random_identifier);
                array_push($listing_photos_1, $random_identifier);
            } else {
                array_push($listing_photos_1, $old_beauty_service_images_1[$key]);
            }
        }

        foreach ($_FILES['new_service_image_2']['tmp_name'] as $key => $service_image_2) {
            if ($service_image_2 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_2, 'uploads/service_images/' . $random_identifier);
                array_push($listing_photos_2, $random_identifier);
            } else {
                array_push($listing_photos_2, $old_beauty_service_images_2[$key]);
            }
        }

        foreach ($_FILES['new_service_image_3']['tmp_name'] as $key => $service_image_3) {
            if ($service_image_3 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_3, 'uploads/service_images/' . $random_identifier);
                array_push($listing_photos_3, $random_identifier);
            } else {
                array_push($listing_photos_3, $old_beauty_service_images_3[$key]);
            }
        }

        foreach ($_FILES['new_service_image_4']['tmp_name'] as $key => $service_image_4) {
            if ($service_image_4 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_4, 'uploads/service_images/' . $random_identifier);
                array_push($listing_photos_4, $random_identifier);
            } else {
                array_push($listing_photos_4, $old_beauty_service_images_4[$key]);
            }
        }

        foreach ($_FILES['new_service_image_5']['tmp_name'] as $key => $service_image_5) {
            if ($service_image_5 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_5, 'uploads/service_images/' . $random_identifier);
                array_push($listing_photos_5, $random_identifier);
            } else {
                array_push($listing_photos_5, $old_beauty_service_images_5[$key]);
            }
        }
        ...
        foreach ($service_ids_array as $key => $service_id) {
            ...
            $beauty_service_data['negotiable'] = sanitizer($service_negotiable_array[$key]);
            $beauty_service_data['service_times'] = sanitizer($service_starting_time_array[$key]) . ',' . sanitizer($service_ending_time_array[$key]) . ',' . sanitizer($service_duration_array[$key]);
            $beauty_service_data['is_featured'] = sanitizer($service_is_featured_array[$key]);
            $beauty_service_data ['photo'] = sanitizer($listing_photos_1[$key]);
            $beauty_service_data ['photo_2'] = sanitizer($listing_photos_2[$key]);
            $beauty_service_data ['photo_3'] = sanitizer($listing_photos_3[$key]);
            $beauty_service_data ['photo_4'] = sanitizer($listing_photos_4[$key]);
            $beauty_service_data ['photo_5'] = sanitizer($listing_photos_5[$key]);
            $beauty_service_data['listing_id'] = sanitizer($listing_id);
            if ($service_id > 0) {
                $this->db->where('id', $service_id);
                $this->db->update('service', $beauty_service_data);
            } else {
                $this->db->insert('service', $beauty_service_data);
            }
        }
    ...
    function update_listing_product_details($listing_id = "")
    {
        // checking1122
        $listing_photos_1 = array();
        $listing_photos_2 = array();
        ...
        $product_name_array = sanitizer($this->input->post('new_product_name'));
        array_pop($product_name_array);

        $category_id_array = $this->input->post('new_category_id');
        array_pop($category_id_array);

        $product_price_array = $this->input->post('new_price');
        array_pop($product_price_array);

        $product_unit_array = $this->input->post('new_unit');
        array_pop($product_unit_array);

        $product_color_array = $this->input->post('new_color');
        array_pop($product_color_array);

        $product_brand_array = $this->input->post('new_brand');
        array_pop($product_brand_array);

        $product_size_specification_array = $this->input->post('new_size_specification');
        array_pop($product_size_specification_array);

        $product_weight_specification_array = $this->input->post('new_weight_specification');
        array_pop($product_weight_specification_array);

        $product_other_specification_array = $this->input->post('new_other_specification');
        ...
        $product_availability_array = $new_availability_array;
        // $product_availability_array = $this->input->post('availability');
        // array_pop($product_availability_array);
        ...
        // $product_is_featured_array = $this->input->post('is_featured');
        $product_is_featured_array = $new_is_featured_array;
        ...
        $product_ids_array = sanitizer($this->input->post('new_product_id'));
        //  array_pop($product_ids_array);
//            echo '<pre>';
//            print_r($product_ids_array);
//            echo '</pre>';exit;

        // Image Uploading functions starts here

        if(!empty($this->input->post('old_product_images_1'))){

            $old_beauty_service_images_1 = sanitizer($this->input->post('old_product_images_1'));
            array_pop($old_beauty_service_images_1);
        }

        if(!empty($this->input->post('old_product_images_2'))){
            
            $old_beauty_service_images_2 = sanitizer($this->input->post('old_product_images_2'));
            array_pop($old_beauty_service_images_2);
        }

        if(!empty($this->input->post('old_product_images_3'))){
            
            $old_beauty_service_images_3 = sanitizer($this->input->post('old_product_images_3'));
            array_pop($old_beauty_service_images_3);
        }

        if(!empty($this->input->post('old_product_images_4'))){
                
            $old_beauty_service_images_4 = sanitizer($this->input->post('old_product_images_4'));
            array_pop($old_beauty_service_images_4);
        }

        if(!empty($this->input->post('old_product_images_5'))){
            
            $old_beauty_service_images_5 = sanitizer($this->input->post('old_product_images_5'));
            array_pop($old_beauty_service_images_5);            
        }
        ...
        foreach ($_FILES['new_product_image_1']['tmp_name'] as $key => $service_image_1) {
            if ($service_image_1 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_1, 'uploads/shop/' . $random_identifier);
                array_push($listing_photos_1, $random_identifier);
            } else {
                array_push($listing_photos_1, $old_beauty_service_images_1[$key]);
            }
        ...
        foreach ($_FILES['new_product_image_2']['tmp_name'] as $key => $service_image_2) {
            if ($service_image_2 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_2, 'uploads/shop/' . $random_identifier);
                array_push($listing_photos_2, $random_identifier);
            } else {
                array_push($listing_photos_2, $old_beauty_service_images_2[$key]);
            }
        ...
        foreach ($_FILES['new_product_image_3']['tmp_name'] as $key => $service_image_3) {
            if ($service_image_3 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_3, 'uploads/shop/' . $random_identifier);
                array_push($listing_photos_3, $random_identifier);
            } else {
                array_push($listing_photos_3, $old_beauty_service_images_3[$key]);
            }
        ...
        foreach ($_FILES['new_product_image_4']['tmp_name'] as $key => $service_image_4) {
            if ($service_image_4 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_4, 'uploads/shop/' . $random_identifier);
                array_push($listing_photos_4, $random_identifier);
            } else {
                array_push($listing_photos_4, $old_beauty_service_images_4[$key]);
            }
        ...
        foreach ($_FILES['new_product_image_5']['tmp_name'] as $key => $service_image_5) {
            if ($service_image_5 != "") {
                $random_identifier = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($service_image_5, 'uploads/shop/' . $random_identifier);
                array_push($listing_photos_5, $random_identifier);
            } else {
                array_push($listing_photos_5, $old_beauty_service_images_5[$key]);
            }
        ...
        foreach ($product_ids_array as $key => $service_id) {
            $beauty_service_data['name'] = sanitizer($product_name_array[$key]);
            $beauty_service_data['category_id'] = sanitizer($category_id_array[$key]);
            $beauty_service_data['price'] = sanitizer($product_price_array[$key]);
            $beauty_service_data['unit'] = sanitizer($product_unit_array[$key]);
            $beauty_service_data['color'] = sanitizer($product_color_array[$key]);
            $beauty_service_data['brand'] = sanitizer($product_brand_array[$key]);
            $beauty_service_data['size_specification'] = sanitizer($product_size_specification_array[$key]);
            $beauty_service_data['weight_specification'] = sanitizer($product_weight_specification_array[$key]);
            $beauty_service_data['details'] = sanitizer($product_other_specification_array[$key]);
            $beauty_service_data['availability'] = sanitizer($product_availability_array[$key]);
            $beauty_service_data['is_featured'] = sanitizer($product_is_featured_array[$key]);

            
            $beauty_service_data ['thumbnail'] = sanitizer($listing_photos_1[$key]);
            $beauty_service_data ['thumbnail_2'] = sanitizer($listing_photos_2[$key]);
            $beauty_service_data ['thumbnail_3'] = sanitizer($listing_photos_3[$key]);
            $beauty_service_data ['thumbnail_4'] = sanitizer($listing_photos_4[$key]);
            $beauty_service_data ['thumbnail_5'] = sanitizer($listing_photos_5[$key]);
            $beauty_service_data['listing_id'] = sanitizer($listing_id);
            if ($service_id > 0) {
                $this->db->where('id', $service_id);
                $this->db->update('inventory', $beauty_service_data);
            } else {
                $this->db->insert('inventory', $beauty_service_data);
            }
        }

        $this->session->set_flashdata('flash_message', get_phrase('Products Updated successfully.'));
    ...
    function update_listing_type_wise_details($listing_type = "", $listing_id = "")
    {
        ...
        if ($listing_type == 'hotel') {
            ...
            print_r($this->input->post('old_food_menu_images'));
            exit();
            // Image Uploading functions starts here
            if(!empty(sanitizer($this->input->post('old_food_menu_images')))){ 
...
// Currency table queries and operations
    ...
    function get_application_details()
    {
        ...
        $returnable_array = array(
            'purchase_code_status' => 'Licensed to Mark8 Hub',
            'support_expiry_date' => '2050-12-02',
            'customer_name' => 'Mark8 Hub'
        );
...
//new
    public function service_checking_time($param1 = '', $param2 = '')
    {
        $times = $this->db->get_where('service', array('id' => $param1))->row('service_times');
        $time = explode(',', $times);
        $starting_hour_and_minute = explode(':', $time[0]);
        $ending_hour_and_minute = explode(':', $time[1]);
